...
function i2pAdd(input) {
  for (const f of input.files) i2pFiles.push(f);
  renderI2pList();
}
...
/* ══════════════════════════════════════════════
   EDITOR
   ══════════════════════════════════════════════ */
let _pageCanvasDims = {};          /* stores { w, h } per page for correct export scaling */
...
async function renderPage(pageNum) {
  ...
  const natH = page.getViewport({ scale: 1 }).height;
  ...
  /* ── Critical fix ─────────────────────────────────────────────
     If this page was already rendered before at a DIFFERENT pixel
     resolution (browser window resized, sidebar/layers panel changed
     the available width, mobile rotation, etc.), the canvas is about
     to be reset to a new width/height below. Every existing annotation
     on this page has x/y/w/h/points stored in the OLD resolution's
     pixel space — without rescaling them here they silently drift to
     the wrong spot the next time they're drawn or exported (this was
     the real cause of text/signatures/redactions "moving" on save).
     Rescale them proportionally so they stay in the same relative
     spot on the page. ── */
  const oldDims = _pageCanvasDims[pageNum];
  if (oldDims && oldDims.w > 0 && oldDims.h > 0 && (oldDims.w !== vp.width || oldDims.h !== vp.height)) {
    const rw = vp.width / oldDims.w, rh = vp.height / oldDims.h;
    editAnnotations.forEach(ann => {
      if (ann.pageNum !== pageNum) return;
      if (typeof ann.x === 'number') ann.x *= rw;
      if (typeof ann.y === 'number') ann.y *= rh;
      if (typeof ann.w === 'number') ann.w *= rw;
      if (typeof ann.h === 'number') ann.h *= rh;
      if (typeof ann.fontSize === 'number') ann.fontSize *= (rw + rh) / 2;
      if (ann.rect) { ann.rect.x *= rw; ann.rect.y *= rh; ann.rect.w *= rw; ann.rect.h *= rh; }
      if (Array.isArray(ann.points)) ann.points = ann.points.map(p => ({ x: p.x * rw, y: p.y * rh }));
    });
  }

  mc.width=ac.width=vp.width; mc.height=ac.height=vp.height;
  _pageCanvasDims[pageNum] = { w: vp.width, h: vp.height };
    ...
    .forEach(ann=>{
      ...
      if (ann.type==='highlight' && ann.rect) {
        el.style.cssText=
          ...
          `left:${ann.rect.x*sx}px;top:${ann.rect.y*sy}px;`+
          `width:${ann.rect.w*sx}px;height:${ann.rect.h*sy}px;`+
        ...
        el.style.cssText=
          ...
          `left:${ann.x*sx}px;top:${ann.y*sy}px;`+
          `color:${ann.color};font-size:${ann.fontSize*sx}px;`+
        ...
        /* Mouse-wheel → resize text (legacy kept) */
        if (sel) {
        ...
        img.style.cssText=`width:${ann.w*sx}px;height:${ann.h*sy}px;display:block;pointer-events:none;image-rendering:high-quality;`;
        el.appendChild(img);
      ...
      el.addEventListener('click', e=>{ e.stopPropagation(); editSelectedId=ann.id; renderOverlays(); });
      ...
      el.addEventListener('pointerdown', e=>{
        ...
        if ((ann.type==='highlight'||ann.type==='redact')&&ann.rect)
          editDragOffset={ x:cx-ann.rect.x, y:cy-ann.rect.y };
        else
          editDragOffset={ x:cx-ann.x, y:cy-ann.y };
        renderOverlays();
      });
      ov.appendChild(el);
    });
...
window.addEventListener('pointermove', e=>{
  ...
  if (editResizing) {
    ...
    if (editResizing.type==='text') {
      /* scale font size proportionally to horizontal drag */
      const ac=getAnnCanvas(), r=ac.getBoundingClientRect();
      const scaleX = ac.width / r.width; /* canvas px per CSS px */
      const delta = dx * scaleX;
      ann.fontSize = Math.max(6, Math.min(400, editResizing.origFontSize + delta * 0.5));
      document.getElementById('edit-font-size').value = Math.round(ann.fontSize);
    } else if (editResizing.type==='signature') {
      const ac=getAnnCanvas(), r=ac.getBoundingClientRect();
      const scaleX = ac.width / r.width;
      const scaleY = ac.height / r.height;
      const newW = Math.max(20, editResizing.origW + dx * scaleX);
      /* maintain aspect ratio */
      const ratio = editResizing.origH / editResizing.origW;
      ann.w = newW;
      ann.h = newW * ratio;
    }
    renderOverlays();
  ...
  if (!ann) return;
  const nx=(e.clientX-r.left)*sX-editDragOffset.x;
  const ny=(e.clientY-r.top)*sY-editDragOffset.y;
  if ((ann.type==='highlight'||ann.type==='redact')&&ann.rect) { ann.rect.x=nx; ann.rect.y=ny; redrawStrokes(); }
  else { ann.x=nx; ann.y=ny; }
  renderOverlays();
});
...
function setupCanvasEvents() {
  ...
  ac.onpointerup = e => {
    ...
    if ((editTool==='highlight'||editTool==='redact') && editCurrentPts.length>=2) {
      ...
      if (rw>4&&rh>4) pushAnnotation({id:uid(),type:editTool==='redact'?'redact':'highlight',pageNum:editCurrentPage,color:editColor,rect:{x:lx,y:ly,w:rw,h:rh}});
      redrawStrokes(); renderOverlays(); renderLayersPanel();
    } else if (editTool!=='eraser'&&editTool!=='redact'&&editCurrentPts.length>=2) {
      pushAnnotation({id:uid(),type:editTool,pageNum:editCurrentPage,color:editColor,points:[...editCurrentPts]});
      renderLayersPanel();
    }
    editCurrentPts=[];
  };
...
function editAskText(pos) {
  ...
  inp.style.cssText=
    `position:absolute;left:${pos.x*sx}px;top:${pos.y*sy}px;`+
    `font-size:${editFontSize*sx}px;font-family:${editFontFamily};line-height:1;`+
    `color:${editColor};background:rgba(255,255,255,.92);`+
    `border:2px dashed ${editColor};border-radius:6px;`+
    `padding:0;outline:none;min-width:140px;`+
    `pointer-events:auto;z-index:20;direction:rtl;`+
  ...
  const commit = ()=>{
    ...
    if (txt) pushAnnotation({id:uid(),type:'text',pageNum:editCurrentPage,x:pos.x,y:pos.y,text:txt,color:editColor,fontSize:editFontSize,fontFamily:editFontFamily});
    inp.remove(); ov.style.pointerEvents='none'; renderOverlays(); renderLayersPanel();
  };
  inp.onblur=commit;
...
function pushAnnotation(ann) { editAnnotations.push(ann); pushHistory(); }
...
function confirmSignature() {
  ...
  const w=ac.width*0.38, h=w*0.28;
  pushAnnotation({id:uid(),type:'signature',pageNum:editCurrentPage,
    x:ac.width/2-w/2, y:ac.height/2-h/2, w, h, dataUrl});
...
async function editExport() {
  ...
  for (const [pnStr, anns] of Object.entries(byPage)) {
    ...
    const dims = _pageCanvasDims[pn] || { w: getAnnCanvas().width || 800, h: getAnnCanvas().height || 1000 };
    const sX = pW / dims.w, sY = pH / dims.h;
    ...
    if (strokes.length) {
      ...
      tc.width  = dims.w * scale; tc.height = dims.h * scale;
      const ctx = tc.getContext('2d');
      strokes.forEach(ann => {
    ...
    /* ② Text & signature annotations → vector */
    for (const ann of anns) {
      if (ann.type === 'text') {
        /* Try to embed custom font if loaded, otherwise use Helvetica */
        let font;
        ...
        if (customName && _customFonts[customName]) {
          try {
            font = await doc.embedFont(new Uint8Array(_customFonts[customName]));
          } catch { font = await doc.embedFont(PDFLib.StandardFonts.Helvetica); }
        } else {
          font = await doc.embedFont(PDFLib.StandardFonts.Helvetica);
        }
        const fs   = Math.max(6, ann.fontSize * sX);
        /* Use the font's real ascent (top-of-glyph → baseline) instead of the full
           font size, so the exported text lines up with where it visually sat in
           the editor (which anchors text boxes at their top-left, padding:0, line-height:1). */
        let ascent;
        try { ascent = font.heightAtSize(fs, { descender: false }); } catch { ascent = fs * 0.8; }
        pdfPage.drawText(ann.text || '', { x: ann.x * sX, y: pH - ann.y * sY - ascent, size: fs, font });
      } else if (ann.type === 'signature') {
        try {
          ...
          pdfPage.drawImage(img, { x: ann.x*sX, y: pH-(ann.y+ann.h)*sY, width: ann.w*sX, height: ann.h*sY });
        } catch {}
      }
    }
  }
